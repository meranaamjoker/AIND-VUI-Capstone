...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    rnn_layer = GRU(units, return_sequences=True, activation='relu')(input_data)
    bn_rnn_layer = BatchNormalization(name='bn_rnn_1')(rnn_layer)
    if recur_layers > 1:
        for i in range (recur_layers - 1):
            rnn_layer = GRU(units, return_sequences=True, activation='relu')(bn_rnn_layer)
            bn_rnn_layer = BatchNormalization(name='bn_rnn_{}'.format(2+i))(rnn_layer)

    # TODO-DONE: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(bn_rnn_layer)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
